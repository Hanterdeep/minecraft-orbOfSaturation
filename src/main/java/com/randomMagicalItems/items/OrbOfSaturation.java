...
//                    RandomMagicalItems.LOGGER.catching(e);
    ...
    public ActionResult<ItemStack> onItemRightClick(ItemStack stackk, World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if (inv.getCurrentItem().getItem() instanceof OrbOfSaturation) {
            ...
            for (int i = 0; i < inv.getSizeInventory() && orb.getSaturationCounter(ownStack) < capacity; i++) {
                ...
                if (stack != null && stack.getItem() instanceof ItemFood) {
                    ...
                    for (; taken < stack.stackSize && !orb.chargeOrb(ownStack, ((ItemFood) stack.getItem()).getHealAmount(stack)); taken++)
                        ;
            ...
            playerIn.sendStatusMessage(new TextComponentString("Current orb charge: " + getSaturationCounter(ownStack) + "/" + capacity));
        }

        return super.onItemRightClick(stackk, worldIn, playerIn, handIn);
    }
}
